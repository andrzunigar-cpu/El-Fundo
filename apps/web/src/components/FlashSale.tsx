            ...
            return (
              <div key={p.id} className="group flex flex-col rounded-xl border border-gray-100 overflow-hidden hover:border-red-200 hover:shadow-sm transition">
                <div className="relative aspect-square bg-gray-50 overflow-hidden">
                  ...
                  />
                  {pct > 0 && (
                    <span className="absolute top-1.5 left-1.5 bg-red-600 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full">
                      -{pct}%
                    </span>
                  )}
                </div>
                <div className="p-2 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto">
                    {pct > 0 && (
                      <p className="text-[10px] text-gray-400 line-through leading-none">${original.toLocaleString('es-CL')}</p>
                    )}
                    <p className="text-sm font-black text-red-600">${final.toLocaleString('es-CL')}</p>
                    <button
                      onClick={() => handleAdd(p)}
                      className={`mt-1.5 w-full text-[11px] font-bold py-1.5 rounded-lg transition flex items-center justify-center gap-1 ${
