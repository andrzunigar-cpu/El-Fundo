...
function getDb() {
    if (!db)
        throw new Error('Database no inicializada. Llama initDatabase() primero.');
    return db;
}
async function initDatabase() {
    const dbPath = path.join(electron_1.app.getPath('userData'), 'elfundo-pos.db');
...
function seedInitialData(db) {
  ...
  `);
    for (const c of cats)
        insertCat.run(c.id, c.name, c.slug, c.sort, now, now);
    const products = [
        // Vacuno
